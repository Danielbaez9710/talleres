...
from libreria import libros

from .models import Author, Book
from .forms import AuthorForm, Book, Libros
...
def listarLibros(request):
    lista2 = Book.objects.all()
    a = "hola mundo"
    context = {'lista':lista2,'a':a,}
    template = loader.get_template('libros/index.html')
    return HttpResponse(template.render(context,request))
...
def create_libro(request):
    ...
    form = Libros(request.POST or None)
    if form.is_valid():
        form.save()
...
def detail_view2(request, id):
    ...
    context['object'] = Libros.objects.get(id = id)
